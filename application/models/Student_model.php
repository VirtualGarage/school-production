...
class Student_model extends MY_Model
{
    ...
    public function getStudentsByArray($array)
    {
        ...
        if (!empty($custom_fields)) {
            foreach ($custom_fields as $custom_fields_key => $custom_fields_value) {
                $custom_fields_value_name = str_replace("(","",$custom_fields_value->name);
                $custom_fields_value_name = str_replace(")","",$custom_fields_value_name);
                $tb_counter = "table_custom_" . $i;
                array_push($field_var_array, 'table_custom_' . $i . '.field_value as ' . $custom_fields_value_name);
                $this->db->join('custom_field_values as ' . $tb_counter, 'students.id = ' . $tb_counter . '.belong_table_id AND ' . $tb_counter . '.custom_field_id = ' . $custom_fields_value->id, 'left');
                $i++;
            }
        }
